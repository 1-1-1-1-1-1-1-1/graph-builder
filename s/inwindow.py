...
'''
# Function
DEFAULT_FUNCTIONS = ['abs(x)',
    "weierstrass_function(a, b[, max_n='default'[, borders=None]])",
    INIT_FUNCTION
    ]
DEFAULT_FUNCTION = DEFAULT_FUNCTIONS[0]
DEFAULT_BORDERS = (-10, 10)

# Approximator
ALL_APPROXIMATORS = ['lagrange_polynomial',
    'newton_polynomial_forward',
    'newton_polynomial_forward_equidistant',
    'cubic_spline']
DEFAULT_APPROXIMATOR = ALL_APPROXIMATORS[0]
DEFAULT_NODES = 30
FORM_ARGS_DEFAULT = "(chebyshev_nodes(*xborders, nodes_n),)"

# Graph's building
BUILD_OPTIONS = ['both', 'func', 'approx']
# Build par. - ?
...
# Other
DEFAULT_IS_TOPMOST = True
DEFAULT_TIMING_INFORM = True

LOGGING_ENABLED = True
LOGGING_FILE = "Info.txt"

TMP_CONFIGS = 'tmp_configs.ini'
'''
...
def get_default(section, obj, *, option='default'):
    """Internal. Returns object, which is one of such: None,
    `obj[section[option]]` if section[option] is of type `int`,
    of type `str` (section[option])."""
    ...
    try:
        ...
        return _default
"""
_default = c['function_defaults']['default']
try:
    _default = int(_default)
    DEFAULT_FUNCTION = DEFAULT_FUNCTIONS[_default]
except:
    try:
        assert (eval(_default)) is None
        DEFAULT_FUNCTION = eval(_default)
    except:
        pass
    DEFAULT_FUNCTION = _default
"""
...
`newton_polynomial_forward_equidistant`. \
Try using `newton_polynomial_forward` instead.")
        return
...
interpolation can be based on their value.
 {s} Build parameters: of form \
"key_1=value_1, key_2=value_2" etc. (can be empty).
...
copyr = Label(pw, text="(c) Copyright the student of M-131, anonym",  # | WAS WRITTEN NOT BY ME",
...
frame31.pack(fill='x'); frame32.pack(fill='x'); frame33.pack(fill='x')
...
def help_message_save_g():
    mbox.showinfo("info", """\
 ◦ Save always — always save the graph. It's name is formed from the entry \
beside as `that`+`number`+`extension` ('.png'?). Example: if in the entry \
is "builds/build_" and last stored there number of such files in folder \
"builds/" is 3 (even if lower numbers don't exist), then the next \
picture will be saved to folder "builds" with name "build_4.png".
 ◦ Don't save — never save.
 ◦ Ask — ask, whether to save it. File's name is asked each time. Default name \
is formed in the way as in "save always".
"""[:-1])
...
help_label = Label(configurations, text='help')
help_label.pack(side='right')

def open_cnf_file():
    from os import startfile
    startfile('window.ini')
...
notify_help = Button(configurations, bitmap="info", command=lambda: \
    mbox.showinfo("Info",
"Whether to notify about the time, spent on building graph and table."),
    width=20)
